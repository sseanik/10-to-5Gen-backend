...
def generate_minutes(db_data, db, meeting_id):
    prompt = make_prompt(
        ...
        db_data["minutes"]["prompt"] if "prompt" in db_data["minutes"] else None,
    )
...
def generate_actions(db_data, db, meeting_id):
    ...
    prompt = make_prompt(
        ...
        db_data["actions"]["prompt"] if "prompt" in db_data["actions"] else None,
    )
...
def generate_tickets(db_data, db, meeting_id):
    prompt = make_prompt(
        ...
        db_data["tickets"]["prompt"] if "prompt" in db_data["tickets"] else None,
    )
...
def generate_next_agenda(db_data, db, meeting_id):
    prompt = make_prompt(
        ...
        db_data["agendas"]["prompt"] if "prompt" in db_data["agendas"] else None,
    )
    tools = [
        {
            "type": "function",
...
def make_prompt(db_data, custom, extra):
    # Summaries
    title = db_data["summaries"]["title"]
    meeting_type = db_data["summaries"]["type"]
    attendees = db_data["summaries"]["attendees"]
